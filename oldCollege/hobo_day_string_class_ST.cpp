...
void readData(int particapation[][COLS], string activityNames[]) {
    ...
    for (int outer = 0; outer < ROWS; outer++) {
        getline(inputFile, activityNames[outer]);
...
void saveHoboDayResults(int particapation[][COLS], string activityNames[], int rowTotals[], int lowLocation, int highLocation) {
    ...
    for (int i = 0; i < maxStringLength; i++) outputFile << " ";
    ...
    for (int outer = 0; outer < ROWS; outer++) {
        outputFile << setw(maxStringLength) << activityNames[outer];
        for (int inner = 0; inner < COLS; inner++) {
